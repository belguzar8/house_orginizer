...
namespace EKTemplate
{
    public class CameraManager : MonoBehaviour
    {
        ...
        {
            mxdistance += 0.1f * Time.fixedDeltaTime * rotateAnimationSpeed * x;

            if (mxdistance < -3f) x = -1;
            if (mxdistance > 3f) x = 1;
        }
        ...
        private IEnumerator ShakeCorouitine(float _duration)
        {
        ...
        {
            cam.fieldOfView = Mathf.Lerp(cam.fieldOfView, cameraFov, Time.fixedDeltaTime * msmoothforfov);
